...
int RectangleManager::AddRectangle(float x, float y, float height, float width, glm::vec3 color)
{
	if (m_numRectangles == m_vertices.capacity()) {
		m_vertices.resize(m_numRectangles * 2);
		m_indices.resize(m_numRectangles * 2 * 6);
	}

	static constexpr unsigned int sizeRect = sizeof(Rectangle)/ sizeof(glm::vec3);
